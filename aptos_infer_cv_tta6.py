...
y_reg_cv_tta = np.zeros([len(model_names), TTA_ROUNDS, len(test_df), NUM_CLASSES])
...
for i_model, model_name in enumerate(model_names):
    ...
    for i_tta in range(TTA_ROUNDS):
        ...
        y_reg_cv_tta[i_model, i_tta, ...] = model.predict_generator(generator=test_gen, verbose=1)

y_ensemble = y_reg_cv_tta.mean(axis=(0, 1))
y_pred = multi_2_label(y_ensemble)
