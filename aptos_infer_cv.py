...
y_reg_cv = np.zeros([len(model_names), len(test_df)])
...
for i_model, model_name in enumerate(model_names):
    ...
    y_reg_cv[i_model, ...] = model.predict_generator(generator=test_gen, verbose=1).flatten()

y_tmp = y_reg_cv.clip(max=4.5).mean(axis=0)
y_pred = KappaOptimizer.regression_2_category(y_tmp, coef=INIT_KAPPA_COEF)
