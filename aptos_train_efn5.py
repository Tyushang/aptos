...
def build_model(trained_model_path=MODEL_PATH,
    ...
    if trained_model_type == 'weights':
        ...
        out = layers.Dense(5, activation='linear', name='out')(x)
        model = models.Model(inp, out, name='aptos-densenet')

        model.compile(
            loss=crossentropy_with_2logits,
...
# y_train_valid = label_counts_2_regression(label_counts, regression_table=REG_TABLE)
y_true_logits_train_valid = label_counts_2_logits(label_counts, )
...
for i_fold, (train_index, valid_index) in enumerate(kf.split(ids, y_true_logits_train_valid)):
    # ## Train data or generator:
    id_train, y_true_logits_train = ids[train_index], y_true_logits_train_valid[train_index]
    train_gen = MyGenerator(
        img_paths=pathj([DATASET_DIR, 'train_images'], id_train, '.png'),
        labels=y_true_logits_train,
    ...
    id_valid, y_true_logits_valid = ids[valid_index], y_true_logits_train_valid[valid_index]
    valid_gen = MyGenerator(
        img_paths=pathj([DATASET_DIR, 'train_images'], id_valid, '.png'),
        labels=y_true_logits_valid,
        batch_size=BATCH_SIZE,
        seqs=[global_seq(), ],
        # rescale=1.0 / 255,
        is_shuffle=False,
    ...
    history = model.fit_generator(
        ...
        validation_data=(x_valid, y_true_logits_valid),
        callbacks=[
...
y_true_logits_train = eval('y_true_logits_train')
y_true_logits_valid = eval('y_true_logits_valid')
...
y_pred_logits_valid = model_best.predict(x_valid, verbose=1)
y_pred_cate_valid = np.argmax(y_pred_logits_valid, axis=-1)
