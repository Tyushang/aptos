...
for i_fold, (train_index, valid_index) in enumerate(kf.split(ids, y_true_reg_train_valid, groups=md5)):
    ...
    train_gen = AdaptiveGenerator(
        img_paths=pathj([DATASET_DIR, 'train_images'], id_train, '.png'),
        labels=list(y_true_reg_train),
    ...
    kappa_metrics = Metrics(adaptive_gen=train_gen, model_save_path=f'model_{i_fold}.h5')
    ...
    history = model.fit_generator(
        ...
        validation_data=(x_valid, y_true_reg_valid),
        callbacks=[
...
y_valid_true = KappaOptimizer.regression_2_category(y_true_reg_valid, coef=INIT_KAPPA_COEF)
